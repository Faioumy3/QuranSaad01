...
const AdminPanel: React.FC<AdminPanelProps> = ({ onLogout }) => {
  ...
  const [replyToLog, setReplyToLog] = useState<{log: StudentLog, studentName: string} | null>(null);
  const [adminReply, setAdminReply] = useState('');
  const [logsFilterStudent, setLogsFilterStudent] = useState('');
  ...
  const handleExportStudentLogsCSV = async (filterStudentCode: string = '', filterDateStr: string = '') => {
    setLoading(true);
    ...
    let csv = '\uFEFFالتاريخ,اليوم,اسم الطالب,الكود,الحفظ الجديد,المراجعة,التسميع,الهدف,ملاحظات\n';
    Object.entries(allData.studentLogs).forEach(([code, logs]) => {
       // فلترة حسب الطالب
       if (filterStudentCode && code !== filterStudentCode) return;
       ...
       logs.forEach(log => {
          ...
          if (filterDateStr) {
            ...
            if (logDate !== filterDateStr) return;
          }
          
          const logDateObj = new Date(log.date);
          const dayName = logDateObj.toLocaleDateString('ar-EG', { weekday: 'long' });
          const dateStr = logDateObj.toLocaleDateString('ar-EG');
          csv += `"${dateStr}","${dayName}","${name}","${code}","${(log.newMemorizing || '').replace(/"/g, '""')}","${(log.review || '').replace(/"/g, '""')}","${log.listening}","${(log.newTarget || '').replace(/"/g, '""')}","${(log.notes || '').replace(/"/g, '""')}"\n`;
       });
    });
    const fileName = `student_logs_${filterStudentCode || 'all'}_${filterDateStr || new Date().toISOString().split('T')[0]}.csv`;
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
  ...
  const handleReplyToLog = async () => {
    if (!adminReply.trim() || !replyToLog) return alert('اكتب الرد أولاً');
    const message = {
      senderId: 'admin',
      senderName: 'الإدارة',
      senderRole: 'admin' as const,
      recipientId: replyToLog.log.studentCode,
      recipientRole: 'student' as const,
      subject: `رد على يومية ${replyToLog.log.dateDisplay}`,
      content: `📋 رد الإدارة على يوميتك بتاريخ ${replyToLog.log.dateDisplay}:\n\n${adminReply}\n\n---\nملخص اليومية:\n• الحفظ: ${replyToLog.log.newMemorizing}\n• المراجعة: ${replyToLog.log.review}\n• الهدف: ${replyToLog.log.newTarget}`,
      timestamp: Date.now(),
      read: false,
      replies: []
    };
  ...
  return (
    <div className="pb-20">
      <div className="bg-white p-6 rounded-2xl shadow-sm mb-6 flex flex-col justify-between items-center gap-4 text-center">
        <div><h2 className="text-2xl font-bold text-secondary">لوحة الإدارة</h2><p className="text-primary mt-1">{new Date().toLocaleDateString('ar-EG', { weekday: 'long' })}، {new Date().toLocaleDateString('ar-EG')}</p></div>
      ...
      <div className="flex flex-wrap gap-2 mb-6 justify-center">
        ...
        <Button variant="secondary" onClick={() => setShowMessages(true)} className="text-sm px-3">✉️ رسائل</Button>
      </div>
      {activeTab === 'dashboard' && (
        <>
          <Card className="mb-6">
                      ...
                      {filteredRecords.map((r, idx) => (
                          <div key={idx} className="p-3 bg-gray-50 rounded-lg border flex flex-col gap-1">
                              ...
                              <div className="flex justify-between text-xs text-gray-500">
                                  ...
                                  <span>📅 {(r as any).dateDisplay || r.date}</span>
                              </div>
             ...
             <div className="space-y-4 max-h-[500px] overflow-y-auto custom-scrollbar">
               {Object.values(teachers).map((t: Teacher) => (
                 <div key={t.code} className="p-4 border rounded-lg flex justify-between items-center bg-gray-50">
                   ...
                   <Button className="p-2 h-10 w-10 flex items-center justify-center shrink-0 mr-2 text-lg" variant="danger" onClick={() => handleDeleteTeacher(t.code)}>🗑️</Button>
                 </div>
               ))}
             </div>
          </Card>
      ...
      {activeTab === 'reports' && (
        <Card title="📊 تقارير اليوميات">
          <div className="space-y-4">
            <div className="bg-gray-50 p-4 rounded-lg space-y-3">
              <h4 className="font-bold text-secondary mb-2">🔍 خيارات الفلترة</h4>
              <div>
                <label className="block text-right mb-1 text-sm font-medium">فرز حسب الطالب:</label>
                <select 
                ...
                >
                  ...
                  {students.filter(s => s.code && s.code !== '---').map(s => (
                    <option key={s.id} value={s.code}>{s.name}</option>
                  ))}
                </select>
              </div>
              <div>
                <label className="block text-right mb-1 text-sm font-medium">اختيار تاريخ معين:</label>
                <input 
                  type="date" 
              ...
              <Button 
                variant="secondary" 
                onClick={() => {setLogsFilterStudent(''); setLogsFilterDate('');}} 
                className="text-sm"
              >
                🔄 إزالة الفلاتر
              </Button>
            </div>
            <Button onClick={() => handleExportStudentLogsCSV(logsFilterStudent, logsFilterDate)} variant="primary" fullWidth>
              <Download className="ml-2 w-4 h-4" /> تحميل يوميات الطلاب (Excel)
            </Button>
            <div className="text-sm text-gray-500 text-center mt-2">
              {logsFilterStudent || logsFilterDate ? 
                `📌 سيتم تصدير: ${logsFilterStudent ? students.find(s=>s.code===logsFilterStudent)?.name || 'طالب محدد' : 'جميع الطلاب'} ${logsFilterDate ? '- تاريخ: ' + new Date(logsFilterDate).toLocaleDateString('ar-EG', {weekday: 'long'}) + '، ' + new Date(logsFilterDate).toLocaleDateString('ar-EG') : ''}` 
                : '📌 سيتم تصدير جميع اليوميات'}
            </div>
          </div>
        </Card>
      )}
        ...
        <div className="fixed inset-0 bg-black/50 z-[100] flex items-center justify-center p-4">
          <Card className="w-full max-w-md max-h-[80vh] flex flex-col bg-white" title={`يوميات: ${selectedStudentLogs.student.name}`}>
             <div className="overflow-y-auto flex-1 custom-scrollbar p-1 space-y-3">
               {selectedStudentLogs.logs.length===0?<div className="text-center py-10">لا توجد يوميات</div>:selectedStudentLogs.logs.map((log, idx)=>(
                 <div key={idx} className="bg-gray-50 p-3 rounded-xl border">
                    <div className="flex justify-between items-center font-bold text-primary mb-2 border-b pb-1">
                      <span>📅 {log.dateDisplay || new Date(log.date).toLocaleDateString('ar-EG', { weekday: 'long' }) + '، ' + new Date(log.date).toLocaleDateString('ar-EG')}</span>
                      <button 
                        onClick={() => setReplyToLog({log, studentName: selectedStudentLogs.student.name})}
      ...
      {replyToLog && (
        <div className="fixed inset-0 bg-black/50 z-[110] flex items-center justify-center p-4">
          <Card className="w-full max-w-md bg-white" title={`💬 رد على يومية ${replyToLog.studentName}`}>
            <div className="bg-gray-50 p-3 rounded-lg mb-4 text-sm">
              <div className="font-bold text-primary mb-1">📅 {replyToLog.log.dateDisplay}</div>
              <div>الحفظ: {replyToLog.log.newMemorizing}</div>
              <div>المراجعة: {replyToLog.log.review}</div>
            </div>
            <div className="mb-4">
              <label className="block mb-2 text-right font-medium">رسالتك للطالب:</label>
              <textarea 
                value={adminReply} 
                onChange={e => setAdminReply(e.target.value)}
                className="w-full p-3 border-2 border-gray-200 rounded-lg focus:outline-none focus:border-primary text-right h-32"
                placeholder="اكتب ملاحظاتك أو تشجيعك للطالب..."
              />
            </div>
            <div className="flex gap-2">
              <Button fullWidth onClick={handleReplyToLog} disabled={loading}>
                {loading ? '...' : '📤 إرسال'}
              </Button>
              <Button fullWidth variant="secondary" onClick={() => {setReplyToLog(null); setAdminReply('');}}>
                إلغاء
              </Button>
            </div>
          </Card>
        ...
        <div className="fixed inset-0 bg-black/50 z-[100] flex items-center justify-center p-4 overflow-y-auto">
          <div className="my-auto">
            <MessagesPanel 
              ...
              userRole="admin"
              recipients={[
                ...Object.values(teachers).map((t: any) => ({ id: t.code, name: t.name, role: 'teacher' as const })),
